	...
	m.containerMap.Visit(func(visitPodUID, visitContainerName, visitContainerID string) {
		if visitPodUID == podUID && visitContainerName == cntName {
			...
			if m.containerRunningSet.Has(visitContainerID) {
				...
				klog.V(4).Info("Container found in the initial running set", "podUID", podUID, "containerName", cntName, "containerID", visitContainerID)
			}
		}
	})

	if !foundAnyContainer {
		klog.V(4).Info("Container not found in the initial map, assumed NOT running", "podUID", podUID, "containerName", cntName)
		return false
	}

	if !foundRunningContainer {
		klog.V(4).Info("Container found in map but not in running set", "podUID", podUID, "containerName", cntName)
		return false
	}
